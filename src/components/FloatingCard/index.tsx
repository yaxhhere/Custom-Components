...
export default function FloatingCard({ children }: { children: any }) {
  const [rotation, setRotation] = useState({ rotationX: '0deg', rotationY: '0deg' });
  ...
  const onMouseMove = (event: any) => {
    ...
    const mouseXPercent = ((event.clientX - boundingRec.left) / (boundingRec.right - boundingRec.left)) * 100;
    const mouseYPercent = ((event.clientY - boundingRec.top) / (boundingRec.bottom - boundingRec.top)) * 100;
